...
<div>
    ...
    {{-- Label --}}
    @if ($label)
        <label for="textarea-input-component-id-{{ $model }}" class="block text-lg font-medium {{ $errors->first($model) ? 'text-red-600 dark:text-red-500' : 'text-black dark:text-white' }}">{{ htmlspecialchars_decode($label) }}</label>
    @endif
    ...
    <div class="mt-2.5 relative">
        ...
        <textarea
            ...
            class="border-2 disabled:cursor-not-allowed resize-none focus:!ring-1 block w-full ltr:pr-10 ltr:pl-4 rtl:pl-10 rtl:!pr-4 py-3.5 placeholder:font-normal placeholder:text-[13px] dark:placeholder-zinc-300 text-base font-medium text-zinc-800 dark:text-white rounded-md dark:bg-transparent {{ $errors->first($model) ? 'focus:!ring-red-600 focus:!border-red-600 border-red-500' : 'focus:!ring-primary-600 focus:!border-primary-600 border-gray-300 dark:border-zinc-500' }} scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-zinc-900 dark:scrollbar-track-zinc-600" {{ $attributes }}>
            </textarea>
