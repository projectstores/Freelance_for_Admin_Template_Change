...
<label for="checkbox-input-component-id-{{ $model }}" class="block text-base font-medium tracking-wide mb-2 {{ $errors->first($model) ? 'text-red-600 dark:text-red-500' : 'text-black dark:text-white' }}">{{ htmlspecialchars_decode($label) }}</label>
